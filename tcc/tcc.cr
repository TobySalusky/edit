...
struct TCCState {
	...
	int set_options(char^ str) -> c:tcc_set_options(^this, str);
	int add_include_path(char^ pathname) -> c:tcc_add_include_path(^this, pathname);
	int add_sysinclude_path(char^ pathname) -> c:tcc_add_sysinclude_path(^this, pathname);
	...
	int add_file(char^ filename) -> c:tcc_add_file(^this, filename);
	bool compile_string(char^ buf) -> c:tcc_compile_string(^this, buf) != -1;
	int set_output_type(int output_type) -> c:tcc_set_output_type(^this, output_type);
	int add_library_path(char^ pathname) -> c:tcc_add_library_path(^this, pathname);
	int add_library(char^ libraryname) -> c:tcc_add_library(^this, libraryname);
	int add_symbol(char^ name, void^ val) -> c:tcc_add_symbol(^this, name, val);
	int output_file(char^ filename) -> c:tcc_output_file(^this, filename);
	int run(int argc, char^^ argv) -> c:tcc_run(^this, argc, argv);
