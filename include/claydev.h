// VERSION: 0.13
// NOTE: PULLED FROM [Commit 06167b4]
// * renamed to claydev.h
// NOTE: edited to use raylib types directly (+ includes raylib): Clay_Vector2=Vector2, Clay_BoundingBox=Rectangle, Clay_Color=Color
// NOTE: increased max scroll data count (10 -> 256)
...
typedef Color Clay_Color;

typedef Rectangle Clay_BoundingBox;
...
typedef CLAY_PACKED_ENUM {
	...
	// NOTE:(crust)
    // Clay ran out of capacity in its scroll data array. This limit can be increased with Clay_SetMaxElementCount().
    CLAY_ERROR_TYPE_SCROLL_DATA_CAPACITY_EXCEEDED,
} Clay_ErrorType;
...
void Clay__InitializePersistentMemory(Clay_Context* context) {
	...
	// NOTE:(crust) 10->256   [INTERNAL_ERROR] may just be exceeding 256
