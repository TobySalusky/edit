...
ExportState make_export_state(int max_frames) -> {
                       ...
                       enum AVCodecID codec_id)
{
    ...
    *codec = avcodec_find_encoder(codec_id);
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        c->width    = from_edit.canvas_width;
        c->height   = from_edit.canvas_height;
        /* timebase: This is the fundamental unit of time (in seconds) in terms
         * of which frame timestamps are represented. For fixed-fps content,
         * timebase should be 1/framerate and timestamp increments should be
         * identical to 1. */
        ost->st->time_base = (AVRational){ 1, from_edit.STREAM_FRAME_RATE };
...
static AVFrame *get_audio_frame(OutputStream *ost)
{
    ...
    if (av_compare_ts(ost->next_pts, ost->enc->time_base,
                      from_edit.STREAM_DURATION, (AVRational){ 1, 1 }) > 0)
        return NULL;
...
void fill_yuv_image(c:AVFrame ^pict, int frame_index, int width, int height)
{
	if (export_state.frames_written < export_state.total_frames) {
		...
		//}
		`;
		Image img = from_edit.images.remove_at(0);
		c:c:`
...
void fill_rl_image(c:AVFrame^ pict, int width, int height, List<Image> &imported_images)
{
	Image img = c:GenImageColor(width, height, c:BLACK); // may not be thread safe
	c:c:`
...
int output_video_frame(c:AVFrame ^frame, List<Image>& imported_images)
{
    c:c:`
	...
	;`
	fill_rl_image(c:frame, c:width, c:height, imported_images);
    c:c:`
...
`;
int decode_packet(c:AVCodecContext^ dec, c:AVPacket ^pkt, List<Image>& imported_images)
{
    c:c:`
    int ret = 0;
    ...
    while (ret >= 0) {
        ...
        if (dec->codec->type == AVMEDIA_TYPE_VIDEO)
            `;
            c:ret = output_video_frame(c:frame, imported_images);
            c:c:`
        else
...
struct EncodingDecoding {
	static bool ImportVideoImpl(char^ input_file_name_no_path, float& dec_frame_rate, List<Image>& imported_images) {
        c:c:`
		int ret = 0;
		...
		while (av_read_frame(fmt_ctx, pkt) >= 0) {
			...
			if (pkt->stream_index == video_stream_idx)
				ret = decode_packet(video_dec_ctx, pkt, imported_images);
			else if (pkt->stream_index == audio_stream_idx)
				ret = decode_packet(audio_dec_ctx, pkt, imported_images);
			av_packet_unref(pkt);
			if (ret < 0)
				break;
		}

        float frame_rate = ((float) video_stream->r_frame_rate.num) / (video_stream->r_frame_rate.den);

		/* flush the decoders */
		if (video_dec_ctx)
			decode_packet(video_dec_ctx, NULL, imported_images);
		if (audio_dec_ctx)
        ...
        // Set out time base
        dec_frame_rate = c:frame_rate;

		return false;
	}

	static void ExportVideoImpl(int imgw, int imgh, int framerate, char^ folder_path, char^ output_file_name_no_path) {
		char ^filename;
