...
void HandleClayErrors(Clay_ErrorData errorData) {
	...
	if (errorData.errorType == CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED) {
	...
	} else if (errorData.errorType == CLAY_ERROR_TYPE_DUPLICATE_ID) {
		println("[CLAY-DUPLICATE-ID]");
		...
		if (errorData.errorText.chars != NULL) {
			printf("clay-internal err msg: %.*s\n", errorData.errorText.length, errorData.errorText.chars);
		}
...
struct EditClayApp {
	...
	static void Init() {
		...
		{
			// for (Font font in fonts) { c:UnloadFont(font); } // NOTE: leak...
			fonts.clear();
		}
	...
	static void Tick(fn_ptr<void()> update_fn, fn_ptr<void()> render_after_fn, bool should_toggle_debug_overlay) {
		if (reinitializeClay) {
			...
			EditClayApp.Init(); // TODO: make sure you're increasing elem size/other stuff if necessary :)
			reinitializeClay = false;
		}
		_UpdateDrawFrame(update_fn, render_after_fn, should_toggle_debug_overlay);
	}

	static void _UpdateDrawFrame(fn_ptr<void()> update_fn, fn_ptr<void()> render_after_fn, bool should_toggle_debug_overlay) {
		Vec2 mouseWheelDelta = c:GetMouseWheelMoveV();
		float mouseWheelX = mouseWheelDelta.x;
		float mouseWheelY = mouseWheelDelta.y;

		if (should_toggle_debug_overlay) {
		...
		if (rl.IsMouseButtonDown(0) && !scrollbarData.mouseDown && Clay.PointerOver(Clay__HashString(CLAY_STRING(c"ScrollBar"), 0, 0))) {
			Clay_ScrollContainerData scrollContainerData = Clay.GetScrollContainerData(Clay__HashString(CLAY_STRING(c"MainContent"), 0, 0));
		...
		// TODO: replace GetFrameTime w/ delta_time?
