...
void fill_rl_image(c:AVFrame^ pict, int width, int height)
{
    ...
    Texture tex = c:LoadTextureFromImage(img); // These need to be unloaded after use
	from_edit.imported_textures.add(tex);
	c:c:`
...
struct EncodingDecoding {
	static bool ImportVideoImpl(int framerate, char^ input_file_name_no_path, float& dec_frame_rate) {
        ...
        float tb = ((float) video_stream->r_frame_rate.num) / (video_stream->r_frame_rate.den);
